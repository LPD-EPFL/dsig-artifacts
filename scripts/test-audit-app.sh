...
"$SCRIPT_DIR"/remote-invoker.sh machine1 $NAME-1 ~/dsig/bin/audit-server --local-id 1 $ARGS
"$SCRIPT_DIR"/remote-invoker.sh machine2 $NAME-2 ~/dsig/bin/audit-client --local-id 2 --server-id 1 $ARGS
