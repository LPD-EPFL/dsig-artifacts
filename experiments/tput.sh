...
# for INGRESS_DELTA in {6950..6000..50}; do
    "$SCRIPT_DIR"/test-tput-dsig-wots.sh base haraka 7 2 constant $INGRESS_DELTA
    "$SCRIPT_DIR"/test-tput-dsig-wots.sh base haraka 7 2 exponential $INGRESS_DELTA
done
for INGRESS_DELTA in {100000..70000..10000} {68000..50000..2000} {49000..40000..1000} {39000..35000..500}; do
    "$SCRIPT_DIR"/test-tput-eddsa.sh base dalek constant $INGRESS_DELTA
    "$SCRIPT_DIR"/test-tput-eddsa.sh base dalek exponential $INGRESS_DELTA
done
for INGRESS_DELTA in {100000..70000..10000} {68000..63000..1000} {62000..57500..500}; do
# for INGRESS_DELTA in {60000..57500..500}; do
    "$SCRIPT_DIR"/test-tput-eddsa.sh base sodium constant $INGRESS_DELTA
    "$SCRIPT_DIR"/test-tput-eddsa.sh base sodium exponential $INGRESS_DELTA
done
