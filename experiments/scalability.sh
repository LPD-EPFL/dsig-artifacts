...
for RUN in {1..3}; do
    ...
    for NB_CLIENTS in {12..1}; do
        "$SCRIPT_DIR"/test-scalability-verifiers.sh $SUFFIX dsig $NB_CLIENTS auto 0 --msg_size 32
    done

    for NB_CLIENTS in {12..1}; do
        "$SCRIPT_DIR"/test-scalability-signers.sh $SUFFIX dsig $NB_CLIENTS auto 0 --msg_size 32
    done
